...
plot(NA,NA,xlim=xlims,ylim=c(0,1.2),xaxs='i',yaxs='i',ann=FALSE,axes=FALSE)
points(screening$x, screening$mrna, pch=19, col=screening$color, lwd=0)
arrows(x0=ssmry$x, y0=ssmry$l95, y1=ssmry$u95, angle=90, length=0.03, code=3, lwd=1, col=ssmry$color)
segments(x0=ssmry$x-.25, x1=ssmry$x+.25, y0=ssmry$mean, lwd=1, col=ssmry$color)
axis(side=2, at=(0:4)/4, labels=percent((0:4)/4), lwd=0, lwd.ticks=1, las=2)
abline(h=0,lwd=2)
abline(h=1, lwd=1, lty=3)
abline(v=min(xlims), lwd=2)
...
par(xpd=T)
text(x=txlabs$xmid, y=rep(0,nrow(txlabs)), labels=txlabs$treatment, srt=45, adj=c(1,1), col=txlabs$color, font=1)
par(xpd=F)
mtext(side=3, at=reglabs$xmid, text=reglabs$region, col='black', cex=1, font=1, line=0)
mtext(side=2, line=3, text='relative mRNA')
mtext('B', side=3, cex=2, adj = -0.1, line = 0.5)
...
abline(v=c(4,8,11.5))
abline(h=1,lwd=1,lty=3)
points(markers$x, markers$value, pch=19, col=markers$color, lwd=0)
arrows(x0=msmry$x, y0=msmry$l95, y1=msmry$u95, angle=90, length=0.03, code=3, lwd=1, col=msmry$color)
segments(x0=msmry$x-.25, x1=msmry$x+.25, y0=msmry$mean, lwd=1, col=msmry$color)
mtext('D', side=3, cex=2, adj = -0.1, line = 0.5)
